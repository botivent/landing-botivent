...
export function InstagramModal({ integration, integrationId, isOpen, onClose }: InstagramModalProps) {
  ...
  const handleMetaAuth = () => {
    ...
    window.open(`${API_BASE_URL}/api/integrations/meta/auth?provider=instagram_meta`, '_blank')
  }

  const handleSave = async () => {
    if (!formData.instagram_account_id || !formData.access_token) {
    ...
    try {
      ...
      if (integration?.userData) {
        ...
        toast.success("Configuración de Instagram actualizada correctamente")
      } else {
        const result = await integrationsController.createIntegration({
          provider: 'instagram_meta',
          display_name: integration?.display_name || 'Instagram',
          status: 'disconnected',
          config: formData
        })
        ...
        toast.success("Integración de Instagram creada correctamente")
      }
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white" style={{ backgroundColor: '#E4405F' }}>
              <FaInstagram size={20} />
            </div>
            <div>
              <DialogTitle>
                {integration?.userData ? 'Configurar Instagram' : 'Nueva Integración - Instagram'}
              </DialogTitle>
              <DialogDescription>
                {integration?.userData 
                  ? 'Modifica la configuración de tu integración con Instagram Direct'
                  : 'Configura tu nueva integración con Instagram Direct'
                }
              </DialogDescription>
            </div>
          </div>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Secreto de tu aplicación de Instagram (opcional)
            </p>
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            onClick={handleSave}
            disabled={saving || !formData.instagram_account_id || !formData.access_token}
          >
            {saving ? (
              <>
