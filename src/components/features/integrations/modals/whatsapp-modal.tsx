...
export function WhatsAppModal({ integration, integrationId, isOpen, onClose }: WhatsAppModalProps) {
  ...
  const handleMetaAuth = () => {
    // Aquí iría la lógica para autenticar con Meta
    toast.info("Redirigiendo a Meta para autenticación...")
    window.open(`${API_BASE_URL}/api/integrations/meta/auth?provider=whatsapp_meta`, '_blank')
  }

  const handleSave = async () => {
    if (!formData.phone_number || !formData.business_account_id || !formData.access_token) {
    ...
    try {
      ...
      if (integration?.userData) {
        ...
        const result = await integrationsController.updateIntegration(integration.userData.id, {
          ...
          status: 'disconnected',
          // Aquí irían los datos específicos de WhatsApp
          config: formData
        })
        ...
        toast.success("Configuración de WhatsApp actualizada correctamente")
      } else {
        // Crear nueva integración
        const result = await integrationsController.createIntegration({
          provider: 'whatsapp_meta',
          display_name: integration?.display_name || 'WhatsApp Business',
          status: 'disconnected',
          // Aquí irían los datos específicos de WhatsApp
          config: formData
        })
        ...
        toast.success("Integración de WhatsApp creada correctamente")
      }
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white" style={{ backgroundColor: '#25D366' }}>
              <FaWhatsapp size={20} />
            </div>
            <div>
              <DialogTitle>
                {integration?.userData ? 'Configurar WhatsApp Business' : 'Nueva Integración - WhatsApp Business'}
              </DialogTitle>
              <DialogDescription>
                {integration?.userData 
                  ? 'Modifica la configuración de tu integración con WhatsApp Business API'
                  : 'Configura tu nueva integración con WhatsApp Business API'
                }
              </DialogDescription>
            </div>
          </div>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Número de teléfono de tu cuenta de WhatsApp Business
            </p>
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            onClick={handleSave}
            disabled={saving || !formData.phone_number || !formData.business_account_id || !formData.access_token}
          >
            {saving ? (
              <>
